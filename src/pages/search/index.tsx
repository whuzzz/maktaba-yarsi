...
import { RelevantQueries } from '@/features/search';
...
const SearchPage = ({ data }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  ...
  return (
    <>
      <Head>
        <title>Search - Maktaba YARSI</title>
      ...
      <PageWrapper style="flex">
        ...
        <section className="mt-3 ml-auto w-10/12 px-20 pb-20">
          <NavigationsButton
            onNavigate={setPage}
            value={page}
            defaultPage={[1, page - 1, page + 1, data.length]}
          />
          <DisplayContent content={data[page - 1].text} />
          <NavigationsButton
            onNavigate={setPage}
            value={page}
            defaultPage={[1, page - 1, page + 1, data.length]}
          />
          <RelevantQueries data={data} />
        </section>
      </PageWrapper>
    </>
  );
