...
function ListContent({ data, item, indent, on, heading }) {
  ...
  const listStyle = `${indent} ${on(item.page)} ${
    heading ? 'font-medium text-slate-900 dark:text-slate-200' : 'flex before:dark:text-green-500'
  ...
  return (
    <li
      ...
      role="button"
    >
      {!heading && <MdNavigateNext className="shrink-0 text-xl text-green-600" />}
      {text}
    </li>
  );
}
